...
class Student:
    ...
    def report(self):
        rep = f"Report for student {self.first_name} {self.last_name}\n+===============+===============+\n|     Topic     |    Average    |\n+===============+===============+\n"
        for topic in self.grades:
            rep += f"|{topic: ^15}|{self.compute_average(topic): ^15.2f}|\n"
            rep += "+---------------+---------------+\n"
        return rep
...
class Class:
    ...
    def compute_averages(self):
        cls_topics = {}
        for student in self.students:
            st_topics = student.followed_topics
            for topic in st_topics:
                if topic not in cls_topics:
                    cls_topics[topic] = []
                cls_topics[topic].append(student.compute_average(topic))
        averages = {}
        for topic in cls_topics:
            total_grades = cls_topics[topic][0]
            averages[topic] = sum(total_grades)/len(total_grades)
        return averages
